...
#command_args="-M ${MONITOR_PORT} -N -q -o ServerAliveInterval=20 -o ServerAliveCountMax=3 ${USERNAME}@${HOSTNAME} -i ${KEY_PATH}/autossh_rsa_key"
command_args="-vv -M ${MONITOR_PORT} -fN -o \"PubkeyAuthentication=yes\" -o \"PasswordAuthentication=no\" -i ${KEY_PATH}/autossh_rsa_key ${USERNAME}@${HOSTNAME}"
